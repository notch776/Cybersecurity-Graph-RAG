...
# --- 新增函数：添加新节点到缓存时进行相似度过滤 ---
def add_node_to_cache_with_pruning(node_cache, node_obj, parent_node_name=None, relationship_type=None, is_valuable=0, initial_answer_embedding=None, model=None, similarity_threshold=0.5):
    """添加新节点到缓存，带有基于相似度的剪枝功能"""
    node_name = node_obj.get('name')
    if not node_name:
        return False

    # 只有当节点不存在，或者存在但未被查询过时，才考虑添加/更新摘要
    if node_name not in node_cache or node_cache[node_name].get("is_valuable", 0) == 0:
        # 如果有初始回答embedding和模型，进行相似度过滤
        if initial_answer_embedding is not None and model is not None:
            # 构造节点的上下文描述
            node_context = f"{parent_node_name if parent_node_name else ''} {relationship_type if relationship_type else ''} {node_name}"
            # 获取节点上下文的embedding
            node_embedding = get_text_embedding(node_context, model)
            
            if node_embedding is not None:
                # 计算与初始回答的余弦相似度
                similarity = cosine_similarity(initial_answer_embedding, node_embedding)
                print(f"节点 '{node_name}' 的相似度: {similarity:.4f}")
                
                # 如果相似度低于阈值，不添加到缓存
                if similarity < similarity_threshold:
                    print(f"节点 '{node_name}' 因相似度低于阈值 ({similarity:.4f} < {similarity_threshold}) 被剪枝")
                    return False
        
        # 通过相似度检查或无需检查，继续添加节点
        summary = generate_node_summary(node_obj, parent_node_name, relationship_type)
        if summary:
            # 如果节点已存在但未查询，保留其 is_valuable 状态
            existing_valuable_status = node_cache.get(node_name, {}).get("is_valuable", 0)
            node_cache[node_name] = {
                "summary": summary,
                "is_valuable": is_valuable if node_name not in node_cache else existing_valuable_status
...
# --- 处理节点文本并获取embedding的函数 ---
def get_text_embedding(text, model):
    """获取文本的embedding向量"""
    if not model:
        print("警告：模型未初始化，无法获取embedding")
        return None
    try:
...
# --- 修改扩展handle函数为 handle6，实现新 RAG 流程 ---
def handle6(question: str, model=None, fuzzy_threshold=81, embedding_threshold=0.66, max_nodes=3, max_cypher=4, similarity_threshold=0.5):
    """
    使用 LLM、社区感知、意向子问题跟踪的图谱 RAG 流程。
    新增：初始知识回答和基于向量相似度的节点剪枝功能。
    """
    if not driver:
    ...
    print(f"\n--- 开始处理问题 (handle6): {question} ---")
    print(f"参数: fuzzy={fuzzy_threshold}, embed={embedding_threshold}, max_nodes={max_nodes}, max_cypher={max_cypher}, similarity_threshold={similarity_threshold}")

    # --- 0. 生成基于先验知识的初始回答并获取embedding ---
    print("\n--- 生成基于先验知识的初始回答 ---")
    initial_answer_prompt = f"""请基于你的先验知识，简短地回答以下问题。这是一个初步回答，之后我们将基于图谱知识进行更详细的探索。
    
问题: "{question}"

请直接给出简短回答，不要解释你的回答过程。如果你不确定，请给出最合理的推测。"""

    try:
        completion_initial = client.chat.completions.create(
            model="deepseek-v3-250324",
            messages=[
                {"role": "system", "content": "你是一个网络安全专家，请根据先验知识简短回答用户问题。"},
                {"role": "user", "content": initial_answer_prompt}
            ],
        )
        initial_answer = completion_initial.choices[0].message.content
        print(f"初始回答: {initial_answer}")
        
        # 获取初始回答的embedding
        initial_answer_embedding = None
        if model:
            initial_answer_embedding = get_text_embedding(initial_answer, model)
            print("成功获取初始回答的embedding向量")
        else:
            print("警告：模型未初始化，无法获取初始回答的embedding，将不进行相似度剪枝")
    except Exception as e:
        print(f"生成初始回答时出错: {e}")
        initial_answer = "无法生成初始回答"
        initial_answer_embedding = None
            ...
            if raw_neighbor_records:
                ...
                for record in raw_neighbor_records:
                    ...
                    if neighbor_node_obj:
                        ...
                        # 添加邻居到缓存，使用新的带剪枝功能的函数
                        if add_node_to_cache_with_pruning(
                            node_cache, 
                            neighbor_node_obj, 
                            node_name, 
                            rel_type, 
                            is_valuable=0,
                            initial_answer_embedding=initial_answer_embedding,
                            model=model,
...
if __name__ == "__main__":
    # 初始化SentenceTransformer模型，用于向量相似度计算
    model, device = init_model()
    print(f"使用 {device} 设备进行向量计算")
    ...
    print(f"测试问题: {test_question2}")

    final_response = handle6(
        test_question,
        model,                   # 传入模型进行向量相似度计算
        fuzzy_threshold=81,
        ...
        max_cypher=3,            # 最多迭代3次
        similarity_threshold=0.2 # 相似度剪枝阈值
    )
    print("\n------ 最终回答 ------")
